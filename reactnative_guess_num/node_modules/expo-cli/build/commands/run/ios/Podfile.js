...
exports.getDependenciesFromPodfileLock = getDependenciesFromPodfileLock;
...
function getDependenciesFromPodfileLock(podfileLockPath) {
  _log().default.debug(`Reading ${podfileLockPath}`);

  let fileContent;

  try {
    fileContent = _fsExtra().default.readFileSync(podfileLockPath, 'utf8');
  } catch (err) {
    _log().default.error(`Could not find "Podfile.lock" at ${_chalk().default.bold(podfileLockPath)}. Did you run "${_chalk().default.bold('npx pod-install')}"?`);
  ...
  return (0, _jsYaml().safeLoad)(checksumTail)[EXTERNAL_SOURCES_KEY] || {};
}
...
async function maybePromptToSyncPodsAsync(projectRoot) {
  ...
  if (!isLockfileCreated(projectRoot)) {
    await (0, _CreateApp().installCocoaPodsAsync)(projectRoot);
    return;
...
async function promptToInstallPodsAsync(projectRoot, missingPods) {
  if (missingPods === null || missingPods === void 0 ? void 0 : missingPods.length) {
