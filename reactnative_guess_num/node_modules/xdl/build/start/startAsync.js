...
async function startAsync(projectRoot, _a = {}, verbose = true) {
    ...
    const userData = await internal_1.UserManager.getCachedUserDataAsync();
    if (userData === null || userData === void 0 ? void 0 : userData.userId) {
        ...
        if (!internal_1.UnifiedAnalytics.userId) {
            internal_1.UnifiedAnalytics.identifyUser(userData.userId, // userId is used as the identifier in the other codebases (www/website) running unified analytics so we want to keep using it on the cli as well to avoid double counting users
            {
    ...
    internal_1.UnifiedAnalytics.logEvent('action', {
        organization: exp.owner,
        project: exp.name,
        action: 'expo start',
        source: 'expo cli',
        source_version: internal_1.UnifiedAnalytics.version,
    });
