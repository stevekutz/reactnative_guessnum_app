...
class UserManagerInstance {
    ...
    async getCachedUserDataAsync() {
        ...
        try {
            ...
            if (!accessToken && !(userData === null || userData === void 0 ? void 0 : userData.sessionSecret)) {
                return null;
            }
            return userData;
        }
    ...
    async _getProfileAsync({ currentConnection, sessionSecret, accessToken, }) {
        ...
        if ((!this._currentUser || this._currentUser.userId !== user.userId) &&
            ...
            internal_1.UnifiedAnalytics.identifyUser(user.userId, // userId is used as the identifier in the other codebases (www/website) running unified analytics so we want to keep using it on the cli as well to avoid double counting users
            {
                userId: user.userId,
                currentConnection: user.currentConnection,
                username: user.username,
            ...
            internal_1.Analytics.identifyUser(user.username, {
                userId: user.userId,
                currentConnection: user.currentConnection,
                username: user.username,
                userType: user.kind,
