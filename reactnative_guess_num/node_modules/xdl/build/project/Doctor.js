...
const TerminalLink_1 = require("../logs/TerminalLink");
exports.NO_ISSUES = 0;
exports.WARNING = 1;
exports.ERROR = 2;
exports.FATAL = 3;
...
async function _checkNpmVersionAsync(projectRoot) {
    try {
        ...
        const npmVersion = child_process_1.execSync('npm --version', { stdio: 'pipe' }).toString().trim();
...
async function validateWithSchema(projectRoot, _a, schema, configName, validateAssets) {
    ...
    try {
        ...
        if (e instanceof schemer_1.SchemerError) {
            schemaErrorMessage = `Error: Problem${e.errors.length > 1 ? 's' : ''} validating fields in ${configName}. ${TerminalLink_1.learnMore('https://docs.expo.io/workflow/configuration/')}`;
    ...
    if (validateAssets) {
        try {
            ...
            if (e instanceof schemer_1.SchemerError) {
                assetsErrorMessage = `Error: Problem${e.errors.length > 1 ? '' : 's'} validating asset fields in ${configName}. ${TerminalLink_1.learnMore('https://docs.expo.io/')}`;
...
async function _validateReactNativeVersionAsync(exp, pkg, projectRoot, sdkVersions, sdkVersion) {
    ...
    if (!/expo\/react-native/.test(reactNative)) {
        internal_1.ProjectUtils.logWarning(projectRoot, 'expo', `Warning: Not using the Expo fork of react-native. ${TerminalLink_1.learnMore('https://docs.expo.io/')}`, 'doctor-not-using-expo-fork');
        return exports.WARNING;
    }
